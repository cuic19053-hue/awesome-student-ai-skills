...
> **大学生申报书 AI 生成工具**｜一句话，自动写出规范排版的 Word 申报材料。
>
> 覆盖 **大创申报书、挑战杯申报书、互联网+商业计划书、国家奖学金申请书、国家励志奖学金申请材料、保研推免申请、优秀毕业论文、三下乡社会实践报告、入团申请书** 等 **8 大类 · 35 种**高校学生申报材料。
>
> 基于 **Agent Skills 标准**，支持 WorkBuddy / Claude Code / Codex / Cursor / Trae 等支持本地执行的 AI Agent。
...
| 你要交的材料 | 对应赛道 | 输出物 |
|---|---|---|
| **大创申报书**（创新训练 / 创业训练 / 创业实践） | `innovation_research` 等 3 个 | 含技术路线图、甘特图、算法对比表的完整 Word |
| **挑战杯申报书**（课外学术科技作品） | `challenge_cup` | 含 30+ 篇文献综述、创新点提炼的 Word |
| **互联网+ 商业计划书**（主赛道 / 红旅赛道） | `internet_plus` 等 2 个 | 含商业模式闭环、市场分析的 Word |
| **国家奖学金申请书 / 申请理由** | `national_scholarship` | 含成绩排名、竞赛成果、综合素质论证的 Word |
| 国家励志奖学金 / 校级 / 企业 / 单项奖学金 | 4 个赛道 | 对齐各校评审偏好的 Word |
| **保研推免申请 / 选调生申请** | `graduate_recommendation` 等 2 个 | 含简历式亮点提炼的 Word |
| 优秀毕业论文 / 优秀毕业生 / 优秀学生 | 4 个赛道 | 含答辩要点、成果量化的 Word |
| **三下乡社会实践报告** / 支教 / 政策宣讲 / 科技服务 | 4 个赛道 | 含数据图表的社会实践报告 |
| 入团申请书 / 入团阶段汇报 / 转正申请 | 4 个赛道 | 规范格式的政治材料 |
| 校级 / 院级科研立项、西部计划、公派留学、转专业、应征入伍 | 8 个赛道 | 对应赛道的规范 Word |

> 完整 35 个赛道（含内容量与状态）见下方 [能写哪些申报书](#能写哪些申报书)。
...
**Q：写一份大创申报书要多久？**
A：信息采集约 10 分钟（AI 会逐项追问你的专业排名、竞赛经历、项目构想），生成与排版约 2 分钟，全程 15 分钟内拿到可提交的 Word 文档。

**Q：生成的内容会编造事实吗？**
A：不会。项目内置「信息采集清单 + 追问话术」，AI 会先问清你的真实信息再动笔，缺失信息必须追问，不允许虚构事实。但**提交前请务必人工核实所有数据与政策引用**。

**Q：会排版成 Word 吗？还是只有文字？**
A：直接生成本地 `.docx` 文件，含标准字号、表格、签字栏，部分赛道含技术路线图、甘特图、经费饼图。

**Q：需要付费吗？需要自己配 API Key 吗？**
A：不需要付费，也无需自行配置模型 Key。项目基于 Agent Skills 标准，在支持本地执行的 AI 工具中加载即可使用。

**Q：支持哪些 AI 工具？**
A：WorkBuddy、Claude Code、Codex、Cursor、Trae 等任何支持加载 `SKILL.md` 与本地执行 Python 的 Agent 工具。

**Q：和直接问 ChatGPT 有什么区别？**
A：通用大模型不了解各赛道的评审标准与硬性格式要求，且容易编造事实。本项目把 35 个赛道的评审逻辑、字数硬规则、文献要求、格式规范固化为 2,400 KB 结构化领域知识，运行时按需懒加载。

**Q：我的个人信息会被上传吗？**
A：不会。所有信息仅在你的本地会话中处理，不上传、不存储。

**Q：内容依据是什么？**
A：项目拆解了上百篇国家级金奖 / 一等奖真实申报书（大创、互联网+、挑战杯等），提炼其评审逻辑与格式规范；不包含任何受版权保护的原文段落。
...
## 🎯 经过各大经典申报书的严格训练 (Trained on Classic Award-Winning Cases)

本项目框架绝非凭空捏造，而是**深度拆解了上百篇国家级金奖/一等奖的真实申报书**（涵盖大创、互联网+、挑战杯等核心赛事）。
我们将国奖作品的“痛点拆解”、“商业模式闭环”、“创新点提炼”与“技术路线图设计”等顶尖逻辑，通过复杂的 Prompt 工程固化为 AI 必须遵守的“硬规则”。你生成的不仅是公文，更是对齐顶尖视角的致胜策略。

## 项目业务价值与痛点解决

目前市面上的通用大模型（如 ChatGPT）在处理高度专业化、规范化的长文本公文时，普遍面临两大痛点：**缺乏垂直领域知识（不知道怎么写好）** 和 **严重的幻觉问题（喜欢胡编乱造事实）**。

本项目以中国高校高频申报场景（大创、奖学金、竞赛）作为切入点，打造了一个高可用、具备严格边界控制的 Agent Skills 框架体系。

我们不再是简单地向聊天框丢入一段杂乱的 Prompt，而是构建了一套 **Sovereign Prompt Stack（自治提示词栈）**，让支持 Agent 能力的 AI 工具在运行时，能够自动按需检索并懒加载对应的专业结构化知识。
